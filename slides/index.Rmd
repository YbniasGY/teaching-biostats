...
datos_final <-
  ...
  filter(termino %in%
           c("Almost No Chance", "Highly Unlikely", "Improbable",
             "Probably Not", "About Even",
             "We Believe", "Probably", "Highly Likely",
             "Almost Certainly"))
...
ggplot(datos_resumen,
       aes(x = termino, y = mean_prob, fill = termino)) +
  geom_col(alpha = 0.7) +
...
Lo primero que deberemos hacer es **.bg-purple_light[preparar nuestros datos]** para la posterior visualización: en `{ggplot2}` suele ser en formato tidy
...
Para entender mejor los datos vamos a **.bg-purple_light[realizar un resumen de los datos]** calculando la **.bg-purple_light[media de probabilidad asignada]**
...
Por último, para simplficar los gráficos, vamos a **.bg-purple_light[filtrar solo algunos términos]** (tras convertirlos a factor en función de su media de probabilidad con `fct_reorder()`)
...
ggplot(datos_final,
  ...
  geom_histogram(alpha = 0.85, bins = 18) +
  scale_fill_brewer(palette = "RdBu") +
  facet_wrap(~termino, scale = "free_y", nrow = 3) +
...
Los histogramas en realidad son una **.bg-purple_light[aproximación discreta de los gráficos de densidad]** (asumiendo que los intervalos se pudieran ir haciendo tan pequeños como queramos, generado una curva continua). Las densidades mejoran la robustez al histograma. Para ello usaremos `geom_density()`
...
El resumen a veces puede ser insuficiente, podemos usar `geom_jitter()`, que nos añadirá puntos como un **.bg-purple_light[«gotelé aleatorio»]**
...
Podemos mejorar el **.bg-purple_light[«gotelé aleatorio»]** con  `geom_quasirandom()` del paquete `{ggbeeswarm}` (con `width = ...` controlamos la anchura de lo aleatorio)
