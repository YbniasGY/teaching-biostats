...
<https://dadosdelaplace.github.io/teaching-biostats/slides>

- **.bg-red_light[Evaluación]** de la asignatura
<https://github.com/dadosdelaplace/teaching-biostats/tree/main/eval>

- **.bg-yellow[Scripts]** de la asignatura
<https://github.com/dadosdelaplace/teaching-biostats/tree/main/scripts>

- **.bg-orange[Bibliografía]**: <https://github.com/dadosdelaplace/teaching-biostats/tree/main/biblio>
...
#### [CLASE 15: profundizando en ggplot I](#clase-15)
...
#### [CLASE 16: profundizando en ggplot II](#clase-16)

#### [ENTREGA III (clase 17)](#clase-17)

#### [CLASE 18: introducción a Stata](#clase-18)

#### [CLASE 19: uso básico de Stata](#clase-19)
...
# CLASE 16: profundizando en ggplot II

&nbsp;

### [Diagramas polares](#polar-charts)

### [Histogramas](#histograma)

### [Densidades](#densidad)

### [Boxplots y alternativas](#box-plot)

---

name: polar-charts

# Coordenadas .orange[POLARES]

Los gráficos no tienen porque estar en torno a **.bg-purple_light[coordenadas cartesianas]**, y a veces es habitual presentar datos **estacionales** en **.bg-purple_light[coordenadas polares]**
...
font_add_google(family = "Roboto",
                name = "Roboto")
showtext_auto()
theme_set(theme_minimal(base_family = "Cinzel Decorative"))
theme_update(axis.title.y = element_blank(),
             ...
             plot.title =
               element_text(hjust = 0.5, face = "bold", size = 21),
             plot.subtitle = element_text(hjust = 0.5, size = 13),
             plot.caption = element_text(size = 7),
             plot.background = element_rect(fill = alpha("#F4DA47", 0.1)))
datos <-
  Nightingale %>% 
...
ggplot(datos, aes(x = mes, y = tasa, fill = causa)) + 
  ...
  facet_wrap(~periodo) +
  labs(fill = "Causas",
       title = "Diagrama de causas de mortalidad",
       subtitle = "Periodo: Abril 1854 - Marzo 1855",
...
Pero antes...replica este gráfico de barras (sin tocar tema). Prepara los datos como consideres.
...
ggplot(datos, aes(x = mes, y = tasa, fill = causa)) + 
  ...
  labs(fill = "Causas",
       title = "Diagrama de causas de mortalidad",
       subtitle = "Periodos: Abril 1854 - Marzo 1855",
       caption = "Autor: J. Álvarez Liébana | Data: {HistData}")
...
* **.bg-purple_light[Convierte a tibble]** los datos

*  Selecciona solo las variables de **.bg-purple_light[fecha]** (también mes y año) y las **.bg-purple_light[tasas relativas]** (contienen "rate" en el nombre).
...
* Prepara los datos y convierte a **.bg-purple_light[tidy data]**. Renombra además `Date` como `fecha`, `Month` como `month` y `Year` como `year`
...
*  Vamos **.bg-purple_light[eliminar el sufijo ".rate"]** de las causas, y traducirlas a castellano.
...
datos <-
  datos %>%
  mutate(causa = str_remove_all(causa, ".rate"),
         causa = case_when(causa == "Disease" ~ "infecciosas",
                           causa == "Wounds" ~ "heridas",
                           TRUE ~ "otras"))
datos
```
...
* **.bg-purple_light[Crea una nueva variable]** llamada `periodo` que nos dirá si el dato es antes del 1 de marzo de 1855 o después (antes y después de las intervenciones llevadas a cabo por Nightingale). Tras crearla, conviértela a factor de la manera más adecuada.
...
datos <-
  ...
  mutate(periodo =
           ifelse(fecha >= "1855-04-01",
                  "Abril 1855 - Marzo 1856", "Abril 1854 - Marzo 1855"),
         periodo =
           factor(periodo,
...
La manera más inmediata de representar las tasas por causa es hacer uso de un **.bg-purple_light[sencillo diagrama de barras]** (eje x mes, eje y tasa).
...
Añade **.bg-purple_light[theme_minimal()]** y **.bg-purple_light[compón una gráfica por cada periodo]**
...
Nuestros datos abarcan dos periodos: de abril 1854 a marzo 1855, y de abril 1855 a marzo 1856. Realiza cambios para obtener el siguiente gráfico (eje X ordenado de manera correcta).
...
Debemos usar `fct_relevel()` para reordenar los factores
...
Busca en las capas de coordenadas `coord_...()` como cambiar a coordenadas polares.
...
Reordena de nuevo los meses para replicar el gráfico original
...
Para mejorar la visualización y que no se noten tanto las diferencias entre gajos vamos a **.bg-purple_light[aplicar una escala (raíz cuadrada) en el eje Y]**

.pull-left[


```{r eval = FALSE}
ggplot(datos, aes(x = mes, y = tasa,
                  fill = causa)) + 
  geom_col(alpha = 0.85, width = 1) +
  scale_fill_manual(
    values = c("#e3aeae",
...
Dado que en el gráfico original no hay marcas en el eje Y, vamos **.bg-purple_light[eliminar el eje Y]**
...
Para que sea más legible vamos a **.bg-purple_light[reducir el tamaño de las etiquetas de los meses]** y vamos a darle **.bg-purple_light[etiquetas correctas a los meses]** (como en el original).
...
Por último vamos a añadirle fuentes a los textos, poner la leyenda debajo y a poner un fondo amarillo (con transparencia)

```{r eval = FALSE}
library(sysfonts)
library(showtext)
font_add_google(family = "Cinzel Decorative", name = "Cinzel Decorative")
...
# Variables .orange[continuas]: .green[HISTOGRAMA]

Vamos a analizar un conjunto de datos interesante, que contiene las respuestas a las pregunta **.bg-purple_light[«¿Qué probabilidad (%) asignarías al término ...]**

* **.bg-purple_light[«almost no chance»]**
* **.bg-purple_light[«probable»]**
* **.bg-purple_light[«almost certainly»]**

...con el objetivo de **.bg-purple_light[comprender cómo la gente percibe el vocabulario]** de la probabilidad.
...
Replica el siguiente gráfico (prepara los datos como consideres):

* Fuente: Roboto
* Tamaño de título: 27
* Alpha: 0.75
* Paleta: `scale_fill_brewer(palette = "RdBu")`
...
datos_final <-
  datos_pivot %>%
  mutate(termino = fct_reorder(termino, prob))  %>% filter(!(termino %in% c("Chances Are Slight", "We Doubt", "Very Good Chance", "Better Than Even", "Improbable", "Probable")))
...
Lo anterior es un diagrama de barras y hemos tenido que **.bg-purple_light[perder información]** por el camino (hemos tenido que resumir nuestros datos) dado que una diagrama de barras no nos permite visualizar una **.bg-purple_light[variable continua]**.

.pull-left[

Para ello haremos uso de un **.bg-purple_light[histograma]** con `geom_histogram()`

```{r eval = FALSE}
datos <-
  tibble("x" = rnorm(n = 10000,
                     mean = 3, sd = 2))
ggplot(data = datos,
       aes(x = x)) +
  geom_histogram()
...
```{r echo = FALSE, out.width = "85%"}
datos <- tibble("x" = rnorm(n = 10000, mean = 3, sd = 2))
...
El argumento `bins = ...` nos servirá para personalizar el nímero de barras que queremos. Fíjate que el gráfico es una **.bg-purple_light[aproximación discreta]** de un gráfico de densidad.
...
ggplot(datos_final,
  ...
  geom_histogram(alpha = 0.75, bins = 20) +
  scale_fill_brewer(palette = "RdBu") +
...
# Variables .orange[continuas]: .green[DENSIDAD]


.pull-left[

Los histogramas en realidad son una **.bg-purple_light[aproximación discreta de los gráficos de densidad]** (asumiendo que los intervalos se pudieran ir haciendo tan pequeños como queramos, generado una curva continua). Las densidades mejoran la robustez al histograma. Para ello usaremos `geom_histogram()`
...
* Tamaño de título: 27
* Alpha: 0.3
* Paleta: `scale_fill_brewer(palette = "RdBu")` y `scale_color_brewer(palette = "RdBu")`
...
A veces puede ser interesante **.bg-purple_light[superponer las densidades]**, lo cual lo podemos hacer con `geom_density_ridges()`, del paquete `{ggridges}`.
...
library(ggridges)
ggplot(datos_final,
       aes(x = prob, y = termino, color = termino,
           fill = termino)) +
  geom_density_ridges(alpha = 0.5) +
  scale_fill_brewer(palette = "RdBu") +
  scale_color_brewer(palette = "RdBu") +
...
# Variables .orange[continuas]: .green[BOXPLOTS]

Una opción muy habitual en variables continuas son los **.bg-purple_light[gráficos de cajas y bigotes]** o box-plots, algo que podemos hacer fácilmente con `geom_boxplot()`

.pull-left[

* **.bg-purple_light[Caja]**: lados inferior y superior representan el **.bg-purple_light[primer y tercer cuartil]**. La **.bg-purple_light[altura]** de la caja es el **.bg-purple_light[rango intercuartílico]** 

* **.bg-purple_light[Línea]**: la línea gruesa que divide la caja marca la **.bg-purple_light[mediana]**.

* **.bg-purple_light[Bigotes]**: las líneas (bigotes) que salen de las cajas llegan hasta el primer/último dato que no supere 1.5 veces el rango intercuartílico. Los **.bg-purple_light[puntos alejados representan los datos atípicos]** o outliers.
...
Dado que el resumen a veces puede ser insuficiente, podemos usar `geom_jitter()`, que nos añadirá los puntos como una especie de **.bg-purple_light[«gotelé aleatorio»]**

Ver <https://hausetutorials.netlify.app/posts/2019-02-22-why-we-should-never-use-barplots-use-geomquasirandom-instead/>
...
Podemos mejorar el **.bg-purple_light[«gotelé aleatorio»]** con  `geom_quasirandom()` del paquete `{ggbeeswarm}`
...
# Variables .orange[continuas]: .green[VIOLIN]

.pull-left[

Para solventar los problemas de los box-plots, una alternativa muy popular son los **.bg-purple_light[gráficos de violín]** (en realidad es una densidad reflejada)
...
# Variables .orange[continuas]: .green[RAINCLOUD]

También son muy buena alternativa los **.bg-purple_light[raincloud plots]**, visualizando a la vez los datos y un resumen de los mismos.
